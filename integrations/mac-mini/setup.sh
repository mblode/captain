...
step_tools() {
  ...
  if has claude; then ok "claude $(claude --version 2>/dev/null | head -1)"; else curl -fsSL https://claude.ai/install.sh | bash; fi
  if has codex; then ok "codex"; else npm install -g @openai/codex; fi
...
step_captain() {
  say "Captain"
  if has captain; then ok "captain $(captain --version)"; else npm install -g cmux-captain; fi
...
step_hermes() {
  ...
  if has hermes; then
    ...
    has hermes || die "hermes not on PATH after install; open a new shell and re-run"
  fi
  hermes plugins install claude-subscription-directsdk || todo "plugin install failed; Hermes 0.21.4+ is required (hermes update)"
}
...
step_bot() {
  ...
  mkdir -p "$BIN"
  cp "$KIT/bin/hermes-notify" "$KIT/bin/board-changed" "$BIN/"
  chmod 755 "$BIN/"*
  ok "helpers in $BIN"
  ...
  upsert_env "$env" WEBHOOK_ENABLED true
  upsert_env "$env" WEBHOOK_PORT "$WEBHOOK_PORT"

  # Config: model, approvals, Slack threading, webhook routes.
  ...
  sed -e "s|__SECRET_GITHUB__|${HERMES_ROUTE_SECRET_GITHUB}|" \
      ...
      "$KIT/hermes/captain-bot/config.overlay.yaml" >"$overlay"
  touch "$PROFILE_DIR/config.yaml"
  yq -i ". *= load(\"$overlay\")" "$PROFILE_DIR/config.yaml"
  chmod 600 "$PROFILE_DIR/config.yaml"
  rm -f "$overlay"
  ok "config.yaml merged (model on the Claude plan, manual approvals, github + cmux routes)"
...
step_remote_control() {
  ...
  sed -e "s|__CODE_DIR__|$CODE_DIR|" -e "s|__HOME__|$HOME|g" "$KIT/launchd/$PLIST_LABEL.plist" >"$plist"
...
step_routines() {
  ...
  add_routine() {
    ...
    if grep -qF "$marker" <<<"$existing"; then ok "$marker exists"; return; fi
    hermes -p "$PROFILE" cron create "$@" && ok "$marker" || todo "$marker failed; create it in the dashboard"
  }
  add_routine "[captain:board]" "every 15m" \
    "[captain:board] The board changed. Act on the rows in your context as the captain skill says. Post only what needs Matt." \
    --skill captain --script "$BIN/board-changed" --deliver slack
  add_routine "[captain:morning]" "weekdays at 8:45am" \
    "[captain:morning] Morning summary as the captain skill says: READY TO MERGE with links, then NEEDS YOU, then one line on what ran overnight." \
    --skill captain --deliver slack
  add_routine "[captain:weekly]" "fridays at 4:45pm" \
    "[captain:weekly] Weekly numbers: captain gain --since 7d --json. Three lines, then one suggested change." \
    --skill captain --deliver slack
}
...
step_gateway() {
  ...
  ok "gateway installed; re-run this step after installing new tools (it captures PATH)"
  hermes -p "$PROFILE" doctor || todo "hermes doctor reported problems (above)"
}

step_funnel() {
  say "Tailscale Funnel: publish /webhooks/github only"
  local ts
  ts="$(tailscale_cli)"
  [[ -n "$ts" ]] || { todo "install Tailscale first"; return; }
  if "$ts" funnel status 2>/dev/null | grep -q '/webhooks/github'; then ok "already published"; return; fi
  if ask "Publish https://<this Mac>.ts.net/webhooks/github to the internet (HMAC-checked by Hermes)?"; then
    "$ts" funnel --bg --set-path /webhooks/github "http://127.0.0.1:$WEBHOOK_PORT/webhooks/github"
    "$ts" funnel status
...
step_github_hook() {
  ...
  # shellcheck source=/dev/null
  source "$SECRETS"
  say "GitHub webhook: $repo → https://$host/webhooks/github"
  gh api "repos/$repo/hooks" -X POST \
    ...
    -f "config[url]=https://$host/webhooks/github" \
