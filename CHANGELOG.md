...
- e0409bb: Captain v3: one local chat that runs your coding agents.
...
- e0409bb: Pin Codex workers to GPT-6 Sol (`gpt-6-sol`) by default, run reviews at high effort, and launch the Cursor CLI as `agent` (its current binary name), with `.harness.<name>.bin` in config for older installs.

## 2.1.0

### Minor Changes

- 6bb5755: Bind the merged diff to the approved plan, and report rework at the plan gate.
...
- 81ea92a: Refuse planting issue worktrees in a `linear-god` checkout unless `--repo-path` is set, and make the driver loop work from outside cmux: honest ping errors (including cmux-only sockets), plan gates that join macOS `/private` cwds, and `/captain` skill/approve-with-note as the plan path.

## 2.0.1

### Patch Changes

- fae7199: Fix the auto-pickup eligibility gate, which could not match any ticket.
