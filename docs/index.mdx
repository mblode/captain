...
`captain install` adds the `/captain` skill you drive plus the pipeline skills each brief invokes (`/pr-reviewer` → `/pr-creator` → `/pr-babysitter`), then verifies node, git, claude, cmux, and `LINEAR_API_KEY`. It's idempotent — re-run it any time to re-check.
